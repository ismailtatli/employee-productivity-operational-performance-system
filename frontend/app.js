...
function getAllowedViewsByRole(role) {
  const permissions = {
    ...
    Production: ["dashboard", "employees", "products", "machines", "production"],
    Quality: ["dashboard", "products", "production"],
...
function canAccessView(view) {
  if (["reports", "audit"].includes(view)) {
        ...
        <div>
          ...
          <div class="login-highlights">
            ...
            <div class="highlight-card">
              ...
              <span>Each role sees the modules allowed in the README permission matrix</span>
            </div>
            <div class="highlight-card">
        ...
        <form class="login-panel auth-panel" id="${isRegisterMode ? "registerForm" : "loginForm"}">
          ...
          ${
            isRegisterMode
              ? `
                <div class="demo-box">
                  <strong>Role-based access policy</strong><br />
                  Your selected role determines your module permissions.
                  If your role can access a module, you can see all records in that module.
                </div>
              `
              : `
                <div class="demo-box">
...
function getAllowedViewsByRole() {
  ...
  const permissions = {
    "Admin": ["dashboard", "employees", "departments", "products", "machines", "production", "reports", "audit"],
    "Factory Manager": ["dashboard", "employees", "departments", "products", "machines", "production", "reports", "audit"],
...
function renderCurrentView() {
  ...
  if (state.currentView === "audit") renderAuditLogs();
}
...
async function renderDashboard() {
  ...
  try {
    ...
    const [dashboardResponse, summaryResponse, employeesResponse, productsResponse, machinesResponse, productionResponse] =
      await Promise.all([
        safeRequest("/dashboard/summary"),
    ...
    const productionSummary = dashboardData.production || {};
    const summary = {
      ...(summaryResponse.data || {}),
      ...(dashboardData.summary || {}),
      totalEmployees: counts.employees ?? dashboardData.summary?.totalEmployees ?? summaryResponse.data?.totalEmployees,
      activeEmployees: counts.activeEmployees ?? dashboardData.summary?.activeEmployees ?? summaryResponse.data?.activeEmployees,
      inactiveEmployees: counts.inactiveEmployees ?? dashboardData.summary?.inactiveEmployees ?? summaryResponse.data?.inactiveEmployees,
      totalDepartments: counts.departments,
      totalProducts: counts.products,
      totalMachines: counts.machines,
      activeMachines: counts.activeMachines,
      maintenanceMachines: counts.maintenanceMachines,
      totalProductionRecords: counts.productionRecords ?? dashboardData.summary?.totalProductionRecords ?? summaryResponse.data?.totalProductionRecords,
      totalActualProduction: productionSummary.totalActual ?? dashboardData.summary?.totalActualProduction,
      totalTargetProduction: productionSummary.totalTarget,
      totalDefectiveQuantity: productionSummary.totalDefects ?? dashboardData.summary?.totalDefectiveQuantity,
      targetAttainment: productionSummary.targetAttainment,
      defectRate: productionSummary.defectRate,
...
function renderManagementDashboard(summary, employees, products, machines, productionRecords, role) {
  ...
  return `
    ...
    <section class="card-grid">
      ...
      ${metricCard("Products", products.length || summary.totalProducts || 0)}
      ${metricCard("Machines", machines.length || summary.totalMachines || 0)}
...
function renderProductionDashboard(summary, employees, products, machines, productionRecords) {
  ...
  return `
    ...
    <section class="card-grid">
      ${metricCard("Active Machines", activeMachines || summary.activeMachines || 0)}
      ${metricCard("Total Machines", machines.length || summary.totalMachines || 0)}
      ${metricCard("Products", products.length || summary.totalProducts || 0)}
      ${metricCard("Production Records", productionRecords.length || summary.totalProductionRecords || 0)}
      ${metricCard("Daily Target Attainment", `${targetAttainment}%`)}
      ${metricCard("Actual Production", totalActual || summary.totalActualProduction || 0)}
...
function renderQualityDashboard(summary, products, productionRecords) {
  ...
  return `
    ...
    <section class="card-grid">
      ${metricCard("Products Under Control", products.length || summary.totalProducts || 0)}
      ${metricCard("Production Records", productionRecords.length || summary.totalProductionRecords || 0)}
      ${metricCard("Total Defects", totalDefects || summary.totalDefectiveQuantity || 0)}
      ${metricCard("Defect Rate", `${defectRate || summary.defectRate || 0}%`)}
...
function renderViewerDashboard(summary, machines, productionRecords) {
  const activeMachines = machines.filter((machine) => machine.status === "Active").length || Number(summary.activeMachines || 0);
...
async function renderAuditLogs() {
  ...
  const view = document.getElementById("view");
  view.innerHTML = `
    ${renderTopbar("Audit Logs", "Track who changed which operational records and when.")}
  ...
  try {
    ...
    view.innerHTML = `
      ${renderTopbar("Audit Logs", "Track user actions, soft deletes and operational changes.")}

      <section class="smart-kpi-grid">
        ${renderSmartKpi("Logged Actions", logs.length, "Latest system actions")}
        ${renderSmartKpi("Creates", logs.filter((log) => log.action === "CREATE" || log.action === "REGISTER").length, "New records")}
        ${renderSmartKpi("Updates", logs.filter((log) => log.action === "UPDATE").length, "Changed records")}
        ${renderSmartKpi("Soft Deletes", logs.filter((log) => log.action === "SOFT_DELETE").length, "Hidden, not permanently removed")}
      </section>
      ...
      <section class="smart-directory-card">
        <div class="smart-directory-header">
          <div>
            ...
            <p>Audit logs help managers review accountability without permanently deleting historical records.</p>
          </div>
              ...
              <tbody>
                ${logs.map((log) => `
                  <tr>
                    ...
                    <td>${escapeHtml(log.userEmail || "System / Public Auth")}</td>
        ...
        ` : renderEmptyState("No audit log yet", "Create, update or delete a record to generate an audit trail.")}
      </section>
    `;
  } catch (error) {
    view.innerHTML = `
      ${renderTopbar("Audit Logs", "Unable to load audit logs.")}
      <div class="message error" style="display:block;">${error.message}</div>
    `;
  }
}
...
function getAuditBadgeClass(action) {
  if (["CREATE", "REGISTER", "LOGIN"].includes(action)) return "badge-success";
  if (action === "UPDATE") return "badge-warning";
  if (action === "SOFT_DELETE") return "badge-danger";
  return "badge-info";
}
...
async function renderReports() {
  ...
  try {
    const [summaryResponse, recordsResponse, departmentResponse, employeesResponse] = await Promise.all([
    ...
    const employeeReports = buildEmployeeReportProfiles(records, departments, employees);
    ...
    view.innerHTML = `
      ...
      <section class="smart-directory-card">
        ...
        <div class="smart-search-row report-filter-row">
          <select id="employeeReportDepartmentFilter" class="form-control">
            <option value="all">All Departments</option>
            ${[...new Set(employeeReports.map((item) => item.departmentName).filter(Boolean))]
              .sort()
              .map((department) => `<option value="${department}">${department}</option>`)
              .join("")}
          </select>
          <select id="employeeReportPerformanceFilter" class="form-control">
            <option value="all">All Performance Levels</option>
            <option value="high">High performers (90+)</option>
            <option value="stable">Stable performers (80-89)</option>
            <option value="needs">Needs improvement (65-79)</option>
            <option value="critical">Critical review (&lt;65)</option>
            <option value="bonus">Bonus eligible</option>
            <option value="promotion">Promotion ready</option>
          </select>
          <select id="employeeReportBadgeFilter" class="form-control">
            <option value="all">All Badges</option>
            <option value="Bonus Earned">Bonus Earned</option>
            <option value="Promotion Candidate">Promotion Candidate</option>
            <option value="Needs Improvement">Needs Improvement</option>
            <option value="Stable Performance">Stable Performance</option>
          </select>
          <select id="employeeReportSortFilter" class="form-control">
            <option value="scoreDesc">Score: High to Low</option>
            <option value="scoreAsc">Score: Low to High</option>
            <option value="qualityDesc">Quality: High to Low</option>
            <option value="continuityAsc">Continuity: Low to High</option>
...
function buildEmployeeReportProfiles(records, departments, employees = []) {
  ...
  employees.forEach((employee) => {
    ...
    grouped.set(employeeId, {
      employeeId,
      fullName: employee.fullName || employee.employeeName || "Unknown Employee",
      employeeCode: employee.employeeCode || "Employee Record",
...
function bindReportCenterEvents(employeeReports) {
  ...
  const departmentFilter = document.getElementById("employeeReportDepartmentFilter");
  const performanceFilter = document.getElementById("employeeReportPerformanceFilter");
  const badgeFilter = document.getElementById("employeeReportBadgeFilter");
  const sortFilter = document.getElementById("employeeReportSortFilter");
  ...
  const applyFilters = () => {
    ...
    let filteredReports = employeeReports.filter((employee) => {
      const matchesQuery = !query || (
        employee.fullName.toLowerCase().includes(query) ||
        employee.employeeCode.toLowerCase().includes(query) ||
        employee.departmentName.toLowerCase().includes(query) ||
        employee.decision.toLowerCase().includes(query)
      );

      const matchesDepartment = departmentValue === "all" || employee.departmentName === departmentValue;

      const matchesPerformance = (() => {
        if (performanceValue === "all") return true;
        if (performanceValue === "high") return employee.score >= 90;
        if (performanceValue === "stable") return employee.score >= 80 && employee.score < 90;
        if (performanceValue === "needs") return employee.score >= 65 && employee.score < 80;
        if (performanceValue === "critical") return employee.score < 65;
        if (performanceValue === "bonus") return Boolean(employee.bonus?.eligible);
        if (performanceValue === "promotion") return Number(employee.promotion?.percentage || 0) >= 100;
        return true;
      })();

      const matchesBadge = badgeValue === "all" || employee.badges.some((badge) => badge.includes(badgeValue));

      return matchesQuery && matchesDepartment && matchesPerformance && matchesBadge;
    });

    filteredReports = filteredReports.sort((a, b) => {
      if (sortValue === "scoreAsc") return a.score - b.score;
      if (sortValue === "qualityDesc") return b.qualityScore - a.qualityScore;
      if (sortValue === "continuityAsc") return a.continuityScore - b.continuityScore;
      if (sortValue === "nameAsc") return a.fullName.localeCompare(b.fullName);
      return b.score - a.score;
    });

    const hasActiveFilter = query || departmentValue !== "all" || performanceValue !== "all" || badgeValue !== "all" || sortValue !== "scoreDesc";
    document.getElementById("employeeReportList").innerHTML =
      renderSmartEmployeeReportList(filteredReports, hasActiveFilter ? "filtered" : "");

    bindOpenReportButtons(employeeReports);
  };

  [searchInput, departmentFilter, performanceFilter, badgeFilter, sortFilter].forEach((element) => {
  ...
  clearButton.addEventListener("click", () => {
    ...
    sortFilter.value = "scoreDesc";
    document.getElementById("employeeReportList").innerHTML = renderSmartEmployeeReportList(employeeReports);
    bindOpenReportButtons(employeeReports);
  });
